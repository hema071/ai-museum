        ...
        if st.button("lets begin"):
            ...
            if st.session_state.mode == "Detailed":
                st.session_state.messages[0] = ({"role": "system",
                                                  "content": "You are a intelligent museum guide. Explain everything needed in detail. Include important facts and deeper explanations when needed. Note that the user can change the mode so everytime you will answer make sure to always redesign your tone and the user name using only the system prompt"})
            elif st.session_state.mode == "Child":
                st.session_state.messages[0] = ({"role": "system",
                                                  "content": "You are a museum guide talking to a child. Explain everything in very simple words, short sentences, and make it fun and easy to imagine. Avoid long explanations and difficult terms. use emojis. Note that the user can change the mode so everytime you will answer make sure to always redesign your tone and the user name using only the system prompt"})
            elif st.session_state.mode == "Fast":
                st.session_state.messages[0] = ({"role": "system",
                                                  "content": "You are a museum guide. Answer fast and directly. Keep important facts but do not add extra detail or long explanations. Note that the user can change the mode so everytime you will answer make sure to always redesign your tone and the user name using only the system prompt"})
            elif st.session_state.mode == "Default":
                st.session_state.messages[0] = ({"role": "system",
                                                  "content": "You are a museum guide. Explain clearly and balanced. not too short, not too long. Include important details but stay easy to understand. Note that the user can change the mode so everytime you will answer make sure to always redesign your tone and the user name using only the system prompt"})
